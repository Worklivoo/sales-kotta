...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const result = await planoCreditosExtraService({
    ...
    console.error('Erro na API de compra de creditos extras:', error);
    return response.status(500).json({ error: 'Nao foi possivel comprar creditos extras.' });
  }
}
