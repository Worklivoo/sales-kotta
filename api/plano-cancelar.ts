...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const result = await planoCancelarService({
    ...
    console.error('Erro na API de cancelamento de plano:', error);
    return response.status(500).json({ error: 'Nao foi possivel cancelar o plano.' });
  }
}
